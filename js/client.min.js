...
var y=e[n];e[n]=e[v];e[v]=y;r()}});$(k).each(function(){$(document).bind(this.event||"keypress",this.keys,this.action)})};var a=function(){$(e).each(function(){var w=this,v=$.grep(trackers,function(x){return x.title==w.trackerTitle})[0];if(v){board.createWidget(v,w.queryData)}});b=false};return{load:function(){b=true;o();p=t.plusServer&&t.plusServer==window.location.host;if(!readCookie("lastVisited")){this.displayIntroduction()}createCookie("lastVisited",new Date(),365);if(Modernizr.localstorage){v=localStorage.getItem(f);
